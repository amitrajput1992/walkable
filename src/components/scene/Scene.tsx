...
const Scene: React.FC<SceneProps> = ({ debug = false }) => {
  ...
  // State to store the player position
  ...
  const generateRandomPositionsInNavmesh = (count: number) => {
    ...
    console.info("Generating positions within navmesh bounds:", bounds);
    ...
    while (positions.length < count && attempts < maxAttempts) {
      ...
      try {
        ...
        if (path && path.length > 0) {
          ...
          console.info(
            `Found valid position ${positions.length}/${count}:`,
            position
          );
        }
      } catch (error) {
        console.error(
          "Error finding path in generateRandomPositionsInNavmesh:",
          error
        );
    ...
    console.info(
      `Generated ${positions.length} valid positions after ${attempts} attempts`
    );
    return positions;
  };
  ...
  useEffect(() => {
          ...
          if (positions.length > 0) {
            ...
            const newNpcs = Array.from(
              ...
              (_, i) => ({
                ...
                position: [positions[i].x, 0.5, positions[i].z] as [
                  number,
                  number,
                  number
                ],
            ...
            console.warn(
              "Could not generate valid positions. Using fallback position."
            );
    ...
    simulationStarted,
    scene,
    generateRandomPositionsInNavmesh,
  ]);
  ...
  const handleGroundClick = (event: any) => {
    ...
    console.info("clickedPoint", clickedPoint);
    ...
    scene.traverse((object) => {
      if (
        object.userData &&
        object.userData.isNPC &&
        object.userData.setDestination
      ) {
    ...
    scene.traverse((object) => {
      if (object.userData && object.userData.setDestination) {
        object.userData.setDestination(clickedPoint);
      }
    });
  };
    ...
    <>
      ...
      <React.Suspense fallback={null}>
        ...
        <group ref={groupRef}>
          ...
          <NavMesh
            ref={navMeshRef as any}
            debug={debug}
            handleClick={handleGroundClick}
          />
          ...
          {/* Player with click-to-move functionality */}
          {navMeshRef ? (
            <Player
